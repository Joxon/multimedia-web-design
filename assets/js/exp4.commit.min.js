...
imgCap.html('密码正确。准备上传...');let api=new GithubAPI({token:token});api.setRepo('joxon','multimedia-web-design');api.setBranch('gh-pages').then(()=>{imgCap.html('正在上传...');let contentObj={content:imgBase64Data,encoding:'base64'};api.pushFiles(`uploaded ${imgFile.name} to exp4/upload.jpg`,[{content:contentObj,path:'exp4/upload.jpg'}])}).then(()=>{imgCap.html('上传成功！由于GitHub Pages部署需要时间，网页图片可能不会立即更新。点击“查看原图”或“查看历史”可以立即看到效果。')}).catch(e=>console.error(`Error occured when pushing ${imgFile.name}: ${e}`));buttonUpload.prop('disabled',!1);inputFile.prop('disabled',!1)})})(jQuery)